...
function solve(board) {
    ...
    // PRESUMES(lol) the given sudoku board is valid
    if (solved(board)) {
        return board
    }
    else {
...
function nextBoards(board){ 
    // List[Board]
    // finds the first empty square and generates 9 different boards filling in that square with numbers 1...9
    var res = []
...
function boxesGood(board){
    // transform this everywhere to update resolution
...
function inputIsInvalid(){
    // starting board is invalid or puzzle is not solvable
    for (i = 1; i <= 9; i++){
